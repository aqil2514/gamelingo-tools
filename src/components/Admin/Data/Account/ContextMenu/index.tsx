...
export function EditMenu({ contextMenu, setEditMenu }: { contextMenu: ContextMenuState; setEditMenu: React.Dispatch<React.SetStateAction<boolean>> }) {
  ...
  return (
    <div className="w-1/2 absolute top-36 left-[35%] bg-zinc-700 rounded-xl border-2 border-white p-4">
      {Object.keys(data).length === 0 ? (
        <Loading loading={1} textOn text="Mengambil data user..." />
      ) : (
        <form method="post">
          <Input variant={InputClass.dashboard} name="user-id" forId="userId" disabled label="User Id" defaultValue={data.id} />

          <Input variant={InputClass.dashboard} name="oauth-id" forId="oauthId" disabled label="Oauth Id" defaultValue={data.oauthid} />

          <Input variant={InputClass.dashboard} name="name" forId="username" label="Name" defaultValue={data.name} />

          <Input variant={InputClass.dashboard} name="username" forId="username" label="Username" defaultValue={data.username} />

          <Input variant={InputClass.dashboard} name="email" forId="email" label="Email" defaultValue={data.email} />

          <Input variant={InputClass.dashboard} name="role" forId="role" label="Role" defaultValue={data.role} />

          <Input variant={InputClass.dashboard} name="image" forId="image" label="Image" defaultValue={data.image} />

          <Input variant={InputClass.dashboard} name="created-at" forId="createdat" disabled label="Created" defaultValue={data.createdat?.toLocaleDateString("id-ID", { weekday: "long", day: "2-digit", month: "long", year: "numeric" })} />

          <label htmlFor="password-exist" className="text-white font-bold font-poppins mx-4">
            Password Exist
          </label>
          <input type="checkbox" name="password-exist" id="password-exist" defaultChecked={data.passwordExist} />

          <label htmlFor="password-exist" className="text-white font-bold font-poppins mx-4">
            Account Verified
          </label>
          <input type="checkbox" name="password-exist" id="password-exist" defaultChecked={data.account_verified} />

          <div className="flex justify-center gap-4">
            <Button type="button" className={VariantClass.danger} onClick={() => setEditMenu(false)}>
              Batal
            </Button>
          </div>
        </form>
