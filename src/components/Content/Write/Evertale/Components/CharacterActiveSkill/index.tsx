...
import { blurHandler, focusHandler, notif } from "@/utils/fe";
...
export default function CharacterActiveSkill() {
  ...
  return (
    <div>
      ...
      <div>
        <h6 className="text-white underline font-bold font-poppins my-4">
          Active Skill {1}
        </h6>
        <TextField
          forId={`active-skill-name-${1}`}
          label="Skill Name"
          variant="default-variant-1"
          name={`active-skill-name-${1}`}
        />
        <TypeSkill index={1} />
        <div className="grid grid-cols-3 gap-4">
          <TextField
            forId={`active-skill-spirit-${1}`}
            label="Skill Spirit"
            variant="default-variant-1"
            name={`active-skill-spirit-${1}`}
          />
          <TextField
            forId={`active-skill-target-${1}`}
            label="Skill Target"
            variant="default-variant-1"
            name={`active-skill-target-${1}`}
          />
          <TextField
            forId={`active-skill-tu-${1}`}
            label="Skill TU"
            variant="default-variant-1"
            name={`active-skill-tu-${1}`}
          />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div className="relative">
            <Textarea
              forId={`active-skill-desc-en-${1}`}
              className={TextareaStyle.variant_1}
              name={`active-skill-desc-en-${1}`}
          ...
          <div>
            <Textarea
              forId={`active-skill-desc-id-${1}`}
              className={TextareaStyle.variant_1}
              name={`active-skill-desc-id-${1}`}
              label="Deskripsi"
            />
          </div>
        </div>
      ...
      {newField.map((el) => {
        return (
          <div key={el}>
            ...
            <TextField
              forId={`active-skill-name-${el + 1}`}
              label="Skill Name"
              variant="default-variant-1"
              name={`active-skill-name-${el + 1}`}
            />
            <TypeSkill index={el + 1} />
            <div className="grid grid-cols-3 gap-4">
              <TextField
                forId={`active-skill-spirit-${el + 1}`}
                label="Skill Spirit"
                variant="default-variant-1"
                name={`active-skill-spirit-${el + 1}`}
              />
              <TextField
                forId={`active-skill-target-${el + 1}`}
                label="Skill Target"
                variant="default-variant-1"
                name={`active-skill-target-${el + 1}`}
              />
              <TextField
                forId={`active-skill-tu-${el + 1}`}
                label="Skill TU"
                variant="default-variant-1"
                name={`active-skill-tu-${el + 1}`}
              />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="relative">
                <Textarea
                  forId={`active-skill-desc-en-${el + 1}`}
                  className={TextareaStyle.variant_1}
                  name={`active-skill-desc-en-${el + 1}`}
                  label="Description"
                  onFocus={focusHandler}
                  onBlur={blurHandler}
                />
              ...
              <div>
                <Textarea
                  forId={`active-skill-desc-id-${el + 1}`}
                  className={TextareaStyle.variant_1}
                  name={`active-skill-desc-id-${el + 1}`}
                  label="Deskripsi"
                />
              </div>
            </div>
            {newField.length + 1 === el + 1 && (
              <TrashFill
                data-label={el + 1}
